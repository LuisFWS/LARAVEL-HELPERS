...
if (!function_exists('convertStringToDouble')) {
    ...
    function convertStringToDouble(?string $value, $default = null, $precision = 2)
    {
        ...
        return round((float)str_replace(',', '.', str_replace('.', '', $value)), $precision);
    }
}

if (!function_exists('convertStringToDate')) {

    function convertStringToDate(?string $param, mixed $default = null): mixed
    {
        if (empty($param)) {
            return $default;
        }
    ...
    if (!function_exists('validDateFormat')) {
        ...
        function validDateFormat(?string $date, array|string $format = 'd/m/Y H:i:s'): bool
        {
            if (empty($date)) {
                return false;
            }
    ...
    if (!function_exists('consoleInfo')) {
        function consoleInfo($message, $withTime = false): void
        {
            if (config('app.debug')) {
                $time = $withTime ? "[" . now()->format('Y-m-d H:i:s') . "] " : "";
